...
> 本文记录首版共享内存并行的设计与证据。2026-09-12 的效率优化
> （符号分析并行化、分块更新内核、左视任务 DAG、求解复用）及其实测，
> 见 [13-parallel-efficiency.md](13-parallel-efficiency.md)。
> 本文中"M3 外部更新：一个源超节点处理完成后，将不同目标列分配给线程"
> 现在按列块而非单列分配；"任意消元树任务 DAG 调度"一节所述的右视
> 任务图已被左视（每面板一个任务）实现取代。
