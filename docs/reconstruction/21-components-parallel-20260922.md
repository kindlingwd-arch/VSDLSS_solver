...
在构建之前做了一次独立的静态审查，两处问题已修：

1. **测试不密封。** 原门控为"环境变量或阈值"二选一，而 `two_supply_nets` 用调高阈值的方式强制串行参照。若环境里导出了 `VSDLSS_COMPONENTS_UF=1`（A/B 步骤正是如此），参照运行也会走并集查找路径，测试变成自身对比而空过。改为：环境变量只在首次调用时降低默认阈值，**阈值是唯一门控**，测试显式设置的值始终生效。
2. **越界守卫位置错误。** `tails[c] != sizes[c]` 的一致性检查原本在并行区结束后才做。若并集查找的计数与邻接表不一致、某个 BFS 多走了结点，它会先写穿自己的 `order` 区间、进入相邻分量（另一线程）的区间，破坏内存之后才报错。改为在写入前检查 `tail >= limit` 并置越界标记，该分量的 `tails` 记为 -1，由原有检查捕获。这条路径应当不可达（每个非对角元都做了 union），但守卫必须在写之前。

## 验证状态

**本改动未在本地构建或测试。** 开发环境为 Windows，无 gcc/make/WSL。一次试图在云端环境执行构建的尝试落在了同一台 Windows 机器上，未能编译，因此不构成任何验证数据点。以上仅为静态设计与代码审查。
