...
`main` 位于 `0db32ea`。远端此前所有 `codex/*` 与 `integrate/*` 分支，以及用户两批附件（`vsdlss-refactor` 与 `vsdlss-refactor_2`）的工作均已合入 main，没有待合并分支。本节为当前口径；下文各节按里程碑保留历史记录。

### 已合入的三轮优化

- 第 16 篇（`ec35ee1`）：整合 `codex/relaxed-supernodes-20260913` 的精确最小度选择、商图 AMD（order=5）、并行符号映射、分块面板更新、M3 多 RHS 暂存与左视数值 DAG。排除当时的宽松超节点合并原型：严格编译下出现 M3 面板结构断言失败与 M4 稀疏数值分解失败。
- 第 18 篇合并记录（`cc118f1`、`6b9b996`、`b61acd4`）：第一批附件。符号分析改为列计数加每目标源块列表，取代逐条目更新目标表；数值阶段改为左视加分块稠密内核；M3 改用重新实现的宽松超节点并加测试，M4 保持严格超节点；M3/M4 排序后增加消元树后序。设计背景见第 17 篇。
- 第 20 篇合并记录（`ef9e95d`、`2ff0f8d`、`9f46541`、`0db32ea`）：第二批附件，面向低度结点为主的电源网格。低度消元改为分层位图加内存池邻接表并分块并行；按连通分量 BFS 顺序重编号；连通分量与抽取合并为一遍；规范化与上三角检查并行；求解改为原地消元回放、工作区缓存与按消去树并行。技术细节见附件自带的 [18-powergrid-preprocessing](18-powergrid-preprocessing-20260922.md) 与 [19-single-solve-efficiency](19-single-solve-efficiency-20260922.md)。
- `--dag` / `vsdlss_set_dag_enabled` 保留接口兼容，但不再切换数值路径。默认线程数仍为 1。公共 API、M4 磁盘格式与保存/重开接口不变。

### 默认排序已改为 AMD

自 2026-09-22 第二批附件起，`order=0` 与 `order=5` 均为 AMD，MLD 需显式 `-p 4` / `order=4`。依据是 2287.5 万结点客户度数分布电源网格上的实测：AMD 排序 1.8 s、nnz(L) 1.07 亿；MLD 排序 43.5 s、nnz(L) 1.56 亿。RCM、自然序和精确最小度在 1/4 规模上已无法在 10 分钟或 8 GB 内完成。AMD 改变填充与浮点运算顺序，与 MLD 结果不逐位相同。MLD 仍是与反编译证据对应的重建产物，保留可用。

### 性能现状

两组实测，均来自附件作者环境，仅供参考，未在本地复现：

- 2744 节点三维网格、4 RHS、MLD（第 18 篇）：分解 1 线程 324.759 → 37.687 ms，2 线程 258.325 → 40.209 ms。求解阶段未见一致收益。
- 2287.5 万结点客户度数分布电源网格、AMD、2 线程、1 RHS（第 19 篇）：分解 23.6–24.2 → 20.6–21.6 s，单次求解 2.04 → 1.10–1.26 s，峰值内存 5.5 GB，核心 172.7 万阶，nnz(L) 1.07 亿，后向误差 5e-17。1/4 规模 2 线程分解 5.02 → 4.10–4.17 s，求解 0.41–0.43 → 0.22–0.27 s。

第 19 篇说明全量耗时随规模明显超线性，主要来自其沙箱 VM 的内存特性（超过 2 GB 后首次触碰为 2.5–7 s/GB）；按 1/4 规模线性外推，同类 CPU 普通服务器上 2 线程分解约 20–25 s，属估计值，需在目标机器确认。

### 验证现状

附件作者记录的验证（非本仓库独立复测）：严格告警完整回归、`make sanitizers`（ASan/UBSan）、`OPENMP=0` 完整回归、`make smoke`、`test_m3` 开启 LSan；新增重编号/分块路径与电源网格路径的分配失败注入测试，以及 MLD/AMD × 1/2/4 线程的跨线程逐位一致检查。

未完成，不得记为通过：

- **第二批附件合并后的 main 尚未做过任何构建与测试。** 合并在 Windows 上完成，该环境无 gcc/make/WSL，只做了 patch 与 bundle 的等价性、去重、冲突解决和树差异的静态核对。复现命令见第 20 篇。
- TSan 始终未通过有效验收（libgomp 组合下的同步识别问题，退出码 66），没有 race gate。
- M5 原版兼容性未验证，工具自身输出 original compatibility NOT verified。
- 第 18 篇那次 `make sanitizers` 的完整随机超节点套件曾被人工中止（退出码 130），当时仅完成前 7 个矩阵的有界复测。

### 待办

1. **合并后完整验证**：在 Linux 上跑一遍第 20 篇列出的命令并记录退出码，这是当前最优先项。
2. M5 原版兼容：仍缺原版程序、依赖与真实输入输出样本，阻塞项见下文“仍需证据的 M5 项目”。
3. 有效 race gate：需在支持 OpenMP 的工具组合上完成竞态检查。
4. 连通分量阶段（全量 5.6 s）现在是预处理里最大的一项，BFS 与建邻接表均为串行。可改并行标签传播或并查集，但分量编号与 BFS 顺序必须与线程数无关。
5. 多核实测：核心数值分解在 2 线程下已接近饱和，更高线程数的收益未测。
6. 求解接口：gather/scatter 与两遍消元回放合计约 0.7 s，受内存带宽限制。若允许调用方直接提供按内部编号排列的 RHS 可省去 gather/scatter，需新增接口。
7. MLD 的下一步是分隔器质量而非速度，候选方向为 FM 边界细化与多次二分取优。MLD 已不是默认排序，优先级相应下降。
8. 文档编号：`18-powergrid-preprocessing-20260922.md` 与 `18-refactor-merge-20260922.md` 编号重复，见第 20 篇说明。

## 合并状态（2026-09-10 历史记录）
...
## 历史状态：M4 多列与 M5 验收准备（2026-09-10）
