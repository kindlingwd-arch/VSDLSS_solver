...
- 第 16 篇（`ec35ee1`）：整合 `codex/relaxed-supernodes-20260913` 的精确最小度选择、商图 AMD（order=5）、并行符号映射、分块面板更新、M3 多 RHS 暂存与左视数值 DAG。排除当时的宽松超节点合并原型。
- 第一批附件（`cc118f1`、`6b9b996`）：符号分析改为列计数加每目标源块列表；数值阶段改为左视加分块稠密内核；M3 改用重新实现的宽松超节点，M4 保持严格超节点；M3/M4 排序后增加消元树后序。设计见第 17 篇，记录见 [merge-20260922-a](merge-20260922-a.md)。
- 第二批附件（`ef9e95d`、`2ff0f8d`、`9f46541`、`0db32ea`）：低度消元改为分层位图加内存池邻接表并分块并行；按连通分量 BFS 顺序重编号；连通分量与抽取合并；规范化与上三角检查并行；求解原地消元回放、工作区缓存与树并行。**默认排序改为 AMD**。技术文档见第 18（powergrid）、19 篇，记录见 [merge-20260922-b](merge-20260922-b.md)。
- 第三批附件（`be9dc61`）：大分量按大小比例分配嵌套线程组同时处理（此前两个分量时最多只用 2 个线程）；消元记录压缩到约 33 字节；求解先取齐工作区、局部完成后整体写回；8 MiB 以上分配使用透明大页；连通分量阶段按行区间填充加无锁并查集加并行 BFS（3 线程及以上才启用）。技术文档见第 20 篇（vdd-gnd），记录见 [merge-20260923-c](merge-20260923-c.md)。

### 默认排序为 AMD

自 2026-09-22 第二批附件起，`order=0` 与 `order=5` 均为 AMD，MLD 需显式 `-p 4`。依据是 2287.5 万结点客户度数分布电源网格上的实测：AMD 排序 1.8 s、nnz(L) 1.07 亿；MLD 排序 43.5 s、nnz(L) 1.56 亿。AMD 改变填充与浮点运算顺序，与 MLD 结果不逐位相同。

### 性能现状

均为附件作者环境（2 vCPU 虚拟机）的实测，未在本地复现。第三批相对第二批，全量 2288 万结点、2 线程、1 RHS：

| 用例 | 分解 | 首次求解 | 后续求解 | 峰值内存 | 分解缺页 |
|---|---|---|---|---|---|
| VDD+GND 双网 | 15.0–16.0 → 11.6–12.4 s | 0.92–0.99 → 0.67–0.82 s | 0.79–0.82 → 0.66–0.72 s | 5.3–5.5 → 4.6–4.7 GB | 195 万 → 54 万 |
| 单网 | 20.1–20.4 → 15.9–16.2 s | 1.07–1.16 → 0.84–0.91 s | 0.97–1.07 → 0.74–0.86 s | 5.5 → 4.8 GB | 201 万 → 56 万 |

该虚拟机上访存密集操作波动很大（同一程序先后运行可差 20–50%），以上为交替运行、同一时段所得。核心三角求解已接近其访存上限。

### 验证现状

附件作者记录的验证（非本仓库独立复测）：严格告警完整回归、`make sanitizers`、`OPENMP=0` 完整回归、`make smoke`、LSan；新增 1/3/4 线程连通分量逐位比对、1/2/4 线程分解求解逐位比对、Inf 右端项保护、压缩记录逐位一致、逐分配点失败注入。

未完成，不得记为通过：

- **第二、三批附件合并后的 main 从未构建或测试过。** 合并在 Windows 上完成（无 gcc/make/WSL），只做了静态核对；第三批更因只有 patch 没有 bundle，连交叉校验也无法进行。复现命令见 [merge-20260923-c](merge-20260923-c.md)。
- TSan 始终未通过有效验收，没有 race gate。
- M5 原版兼容性未验证，工具自身输出 original compatibility NOT verified。

### 待办

1. **合并后完整验证**：在 Linux 上跑一遍合并记录列出的命令并记录退出码。当前最优先项。
2. **多核实测**：分量按线程比例分组、连通分量并行（≥3 线程才启用）两项的收益，目前只有 2 核数据，作者明确说需要在多核机器上确认。
3. M5 原版兼容：仍缺原版程序、依赖与真实输入输出样本。
4. 有效 race gate：需在支持 OpenMP 的工具组合上完成竞态检查。
5. 核心三角求解受 L 的读取带宽限制，需换填充更少的核心排序或对大超结点做多 RHS 批量求解才会有明显变化。
6. 求解接口：调用方若能直接按内部编号提供 RHS、接收解，可省去 gather/scatter（每次约 0.25 s），需新增接口。
7. MLD 的下一步是分隔器质量而非速度（FM 边界细化、多次二分取优）。已非默认排序，优先级下降。
