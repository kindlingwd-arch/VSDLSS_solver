...
order：0 默认（AMD），1 RCM，2 自然序，3 最小度，4 MLD，5 AMD。推荐 0 或 5；MLD 需显式传 4。因子创建内部保留求解所需数据，不获取用户输入数组所有权；可对同一因子顺序求解多个 RHS。创建失败 out 为 NULL；公开求解失败保持输出不变；支持 rhs 与 solution 为同一数组。释放函数接受 NULL，释放后指针不得复用。
