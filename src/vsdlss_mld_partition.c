...
/* Greedy best-gain moves under the balance limit, one vertex per pass,
 * ties to the lowest vertex.  gain[v] = external - internal edge weight is
 * maintained incrementally (a move changes it only at the moved vertex and
 * its neighbours), so a pass costs O(n) instead of O(n + edges); the move
 * sequence is the same as recomputing every gain from scratch. */
static vsdlss_status refine(const vsdlss_mld_level *g,signed char *part)
{
    csi n=vsdlss_mld_level_vertices(g),pass,a,b,v,k;
    csi *gain=malloc((size_t)(n?n:1)*sizeof(csi));
    if(!gain)return VSDLSS_ERR_OOM;
    side_weights(g,part,&a,&b);
    for(v=0;v<n;v++){const csi *nb,*wt; csi d,in=0,out=0;
        vsdlss_mld_level_neighbors(g,v,&nb,&wt,&d);for(k=0;k<d;k++)if(part[nb[k]]==part[v])in+=wt[k];else out+=wt[k];
        gain[v]=out-in;}
    for(pass=0;pass<n;pass++){
        csi best=-1,best_gain=0;
        for(v=0;v<n;v++){
            csi w,na=a,nbw=b;
            if(gain[v]<=best_gain)continue;
            w=vsdlss_mld_level_vertex_weight(g,v);
            if(part[v]==0){na-=w;nbw+=w;}else{nbw-=w;na+=w;}
            if(balanced(na,nbw)){best=v;best_gain=gain[v];}
        }
        if(best<0)break;
        {
            const csi *nb,*wt; csi d,w=vsdlss_mld_level_vertex_weight(g,best);
            if(part[best]==0){a-=w;b+=w;}else{b-=w;a+=w;}
            part[best]=(signed char)(1-part[best]);
            gain[best]=-gain[best];
            vsdlss_mld_level_neighbors(g,best,&nb,&wt,&d);
            for(k=0;k<d;k++){csi u=nb[k];
                if(part[u]==part[best])gain[u]-=2*wt[k];else gain[u]+=2*wt[k];}
        }
    }
    free(gain);
    return VSDLSS_OK;
...
vsdlss_status vsdlss_mld_partition_level(const vsdlss_mld_level *g,signed char *part,csi *initial,csi *final)
{
    ...
    csi *link=malloc((size_t)n*sizeof(csi)); if(!link){free(candidate);return VSDLSS_ERR_OOM;}
    ...
    for(seed_index=0;seed_index<5;seed_index++){
        ...
        for(v=0;v<n;v++){candidate[v]=-1;link[v]=0;}
        /* link[v] = weight from v into the grown region, kept incrementally. */
        while(used<total/2){csi best=-1,best_link=-1,k;
            if(chosen>=0&&candidate[chosen]<0)best=chosen;
            else for(v=0;v<n;v++)if(candidate[v]<0){csi l=link[v];if(l>best_link||(l==best_link&&(best<0||orig[v]<orig[best]))){best=v;best_link=l;}}
            if(best<0)break;
            candidate[best]=0;used+=vsdlss_mld_level_vertex_weight(g,best);chosen=-1;
            {const csi *nb,*wt;csi d;vsdlss_mld_level_neighbors(g,best,&nb,&wt,&d);for(k=0;k<d;k++)link[nb[k]]+=wt[k];}
        }
        for(v=0;v<n;v++)if(candidate[v]<0)candidate[v]=1;
        candidate_initial=cut_weight(g,candidate);if(refine(g,candidate)!=VSDLSS_OK){free(candidate);free(link);return VSDLSS_ERR_OOM;}candidate_cut=cut_weight(g,candidate);
...
vsdlss_status vsdlss_mld_project_refine(const vsdlss_mld_level *fine,const signed char *coarse_part,signed char *fine_part)
{
    ...
    n=vsdlss_mld_level_vertices(fine);for(v=0;v<n;v++)fine_part[v]=coarse_part[map[v]];return refine(fine,fine_part);
}

/* Kuhn augmenting path from left vertex u.  `seen` holds a per-search stamp
 * instead of being cleared (the former O(n) reset per left vertex made the
 * separator quadratic).  *first receives the right vertex matched to u. */
static int augment(const vsdlss_mld_level *g,const signed char *part,csi u,csi *mr,
                   csi *seen,csi stamp,csi *first)
{
    const csi *nb,*wt;csi d,k;(void)wt;vsdlss_mld_level_neighbors(g,u,&nb,&wt,&d);
    for(k=0;k<d;k++){csi v=nb[k],dummy;if(part[v]!=1||seen[v]==stamp)continue;seen[v]=stamp;
        if(mr[v]<0||augment(g,part,mr[v],mr,seen,stamp,&dummy)){mr[v]=u;*first=v;return 1;}}
    return 0;
}

vsdlss_status vsdlss_mld_node_separator(const vsdlss_mld_level *g,signed char *part,csi *left,csi *right,csi *separator)
{
    csi n,v,k,*mr=NULL,*ml=NULL,*queue=NULL,*seen=NULL,head=0,tail=0;unsigned char *zl=NULL,*zr=NULL;
    if(!g||!part||!left||!right||!separator)return VSDLSS_ERR_INVALID;
    n=vsdlss_mld_level_vertices(g);
    mr=malloc((size_t)n*sizeof(csi));ml=malloc((size_t)n*sizeof(csi));queue=malloc((size_t)n*sizeof(csi));seen=malloc((size_t)n*sizeof(csi));zl=calloc((size_t)n,1);zr=calloc((size_t)n,1);
    if((!mr&&n)||(!ml&&n)||(!queue&&n)||(!seen&&n)||(!zl&&n)||(!zr&&n)){free(mr);free(ml);free(queue);free(seen);free(zl);free(zr);return VSDLSS_ERR_OOM;}
    for(v=0;v<n;v++){mr[v]=ml[v]=-1;seen[v]=-1;}
    for(v=0;v<n;v++)if(part[v]==0){csi first=-1;if(augment(g,part,v,mr,seen,v,&first))ml[v]=first;}
