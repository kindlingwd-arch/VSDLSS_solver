...
.PHONY: all test test-unit test-io test-ordering test-mld test-m3 test-m4 test-m4-panels test-m5 test-parallel bench-parallel sanitizers clean
...
test_m3: test/test_m3.c $(LIBSRCS) include/vsdlss.h src/vsdlss_m3_internal.h
	$(CC) $(CFLAGS) $(PARFLAGS) -o $@ test/test_m3.c $(LIBSRCS) $(LDLIBS)
...
clean:
	rm -f vsdlss_solve vsdlss_solver test_solver test_io test_ordering test_mld test_m3 test_m4 test_m4_panels test_parallel bench_parallel test_omp_tsan_probe src/*.o test_sparse.*
