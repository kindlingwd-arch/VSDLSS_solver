...
暂不支持一般 LU、不定 LDLT、原版 MLD、超节点、磁盘分块、GPU、并行和原版 37 参数 ABI。

## 与反编译原理的对应

| 反编译中确认的阶段 | 新实现 |
|---|---|
| 排序 `MLDOrder/minDegreeOrder` | M1 使用可验证的 RCM 或自然序；MLD 留待后续按证据恢复 |
...
```bash
./vsdlss_solver [-p 0|1|2] [--demo-rhs] jobname
```

`-p 0` 和 `-p 1` 使用 RCM，`-p 2` 使用自然序。默认要求 `.rhs` 存在且长度准确；只有显式指定 `--demo-rhs` 才会在 RHS 缺失时构造 `b=A·1`。成功后写出 `<jobname>.rsl`。
