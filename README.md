...
最新合并与实测结果见 [2026-09-21 优化整合记录](docs/reconstruction/16-main-integration-20260921.md)。主线采用严格超节点，AMD 通过 `-p 5` 显式启用，默认仍为 MLD。
...
实验性数值 DAG 为左视，每个面板一个任务。固定排序下的面板内核改动保持逐位一致（AMD 改变排序，仅要求残差/解误差一致）：
